        ...
        foreach ($list_cats as $cat) {
            if (empty($cat['parentid'])) {
                ...
                if (!empty($cat['subcats'])) {
					...
					$html_content2 = nv_smenu_blocks_hassub($block_config['block_name'], $list_cats, $cat['subcats'], $submenu_active, $block_theme);
                    $xtpl->assign('HTML_CONTENT2', $html_content2);
                    if ($html_content != '') {
						$xtpl->assign('CAT2', $cat);
						$xtpl->parse('main.loopcat1.cat2.nosub');
						if($html_content2 != ''){
							$xtpl->parse('main.loopcat1.cat2.hassub');
						}
                        $xtpl->parse('main.loopcat1.expand');
						
						if($cat['image'] !=''){
							$xtpl->parse('main.loopcat1.cat2.hasimage');
						}
 ...
 * @return string
 */
function nv_smenu_blocks_mainhassub($style, $list_cats, $list_sub, &$submenu_active, $block_theme)
{
    $xtpl = new XTemplate($style . '.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/menu');
    ...
    foreach ($list_cats as $cat) {
        ...
        if (in_array((int) $catid, $list, true) and $list_cats[$catid]['subcats'] != '' ) {
            ...
            $xtpl->assign('MENUTREE', $list_cats[$catid]);
            if (!empty($list_cats[$catid]['icon'])) {
                $xtpl->parse('maintreehassub.icon');
            }
            if (!empty($list_cats[$catid]['subcats'])) {
                $html_content2 = nv_smenu_blocks_hassub($style, $list_cats, $list_cats[$catid]['subcats'], $submenu_active, $block_theme);

                $xtpl->assign('HTML_CONTENT2', $html_content2);
                $xtpl->parse('maintreehassub.tree_content');
            }

            $xtpl->parse('maintreehassub');
        }
    }

    return $xtpl->text('maintreehassub');
}
 ...
 * @return string
 */
function nv_smenu_blocks_hassub($style, $list_cats, $list_sub, &$submenu_active, $block_theme)
{
    $xtpl = new XTemplate($style . '.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/menu');
    ...
    foreach ($list_cats as $cat) {
        ...
        if (in_array((int) $catid, $list, true) and $list_cats[$catid]['subcats'] != '' ) {
            ...
            $xtpl->assign('MENUTREE', $list_cats[$catid]);
            if (!empty($list_cats[$catid]['icon'])) {
                $xtpl->parse('treehassub.icon');
            }
            if (!empty($list_cats[$catid]['subcats'])) {
                $tree = nv_smenu_blocks($style, $list_cats, $list_cats[$catid]['subcats'], $submenu_active, $block_theme);

                $xtpl->assign('TREE_CONTENT', $tree);
                $xtpl->parse('treehassub.tree_content');
            }

            $xtpl->parse('treehassub');
        }
    }

    return $xtpl->text('treehassub');
}
