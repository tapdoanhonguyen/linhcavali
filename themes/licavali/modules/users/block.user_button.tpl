...
function loginFormLoad() {
    $.ajax({
        ...
        if ('reload' == a) {
            ...
            $("#guestBlock_{BLOCKID}").html(a);
            $("#guestBlock_{BLOCKID}").show();
            change_captcha()
        }
    });
}
...
<span><a title="{USER.full_name}" class="pointer button user" data-toggle="tip" data-target="#userBlock_{BLOCKID}" data-click="y" style="background-image:url({AVATA})"><span class="">{USER.full_name}</span></a></span>